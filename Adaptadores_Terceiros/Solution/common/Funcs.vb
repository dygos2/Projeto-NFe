...
Public Class Funcs
    ...
    Public Shared Function calcDesc(ByVal campo As String, ByRef xml As XmlDocument, ByRef contPed As Integer, ByVal contItem As Integer, ByVal vItem As Double) As Double
        ...
        Dim descI, descP, descC, percDesc, tIt As Double
        'quantidade do item em questão
        Dim qComIt = xml.GetElementsByTagName("receipt")(contPed).SelectNodes("receipt_item")(contItem).Attributes.ItemOf("quantity").InnerText

        Try
            percDesc = CDbl(xml.GetElementsByTagName("receipt")(contPed).SelectSingleNode("receipt_details").Attributes.ItemOf("discount_total").InnerText) / 100
            descP = Math.Round((vItem) * percDesc, 3) * qComIt 'multiplica pela qtd e itens o % de desconto dado na compra
        Catch ex As Exception
        ...
        For its = 0 To xml.GetElementsByTagName("receipt")(contPed).SelectNodes("receipt_item").Count - 1
            'somatoria de todos os itens
            tIt += (xml.GetElementsByTagName("receipt")(contPed).SelectNodes("receipt_item")(its).Attributes.ItemOf("adjusted_price").InnerText) / 100

        Next its

        Dim totDescPerc = Math.Round(tIt * percDesc, 3)
        Dim tPed = (xml.GetElementsByTagName("receipt")(contPed).SelectSingleNode("receipt_details").Attributes.ItemOf("total").InnerText / 100)
        Dim frete = xml.GetElementsByTagName("receipt")(contPed).SelectSingleNode("receipt_details").Attributes.ItemOf("shipping_cost").InnerText / 100
        If Math.Round(tIt, 2) > (Math.Round((tPed - frete) + totDescPerc, 2)) Then
            'descC ( desconto de cupom )
            descC = tIt - (tPed - frete + totDescPerc)
            'regra de 3 para aplicar desconto respectivo ao item
            descI += (vItem * descC) / tIt

        End If

        Dim desc = (descI * qComIt) + descP
        Return desc
    End Function
