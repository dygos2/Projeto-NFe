...
Public Class EnvioMonitorCont
    ...
    Private Sub enviarTodasNFe()
        ...
        Dim naoProcessadas As List(Of notaVO) = notaDAO_cont.obterNotasNaoEnviadas
        Dim nota As notaVO
        ...
        For Each nota In naoProcessadas
            ...
            Catch ex As Exception
                Log.registrarErro("Erro ao carregar XML (cont): " & pathXmlTransformado, "EnvioService")
                Continue For
            End Try
            ...
            If UfsCont.SVCAN.Contains(empresa.uf) Then
                ufWs = "SVCAN"
                'Alterando o conteúdo do XML
                nfeXML.GetElementsByTagName("tpEmis")(0).InnerXml = 6
            ElseIf UfsCont.SVCRS.Contains(empresa.uf) Then
                ufWs = "SVCRS"
                'Alterando o conteúdo do XML
                nfeXML.GetElementsByTagName("tpEmis")(0).InnerXml = 7
            End If
            '2.00
            ...
            'Alterando o conteúdo do XML
            Dim rootide As XmlNode = nfeXML.GetElementsByTagName("ide")(0)
            Dim dhelem As XmlElement = nfeXML.CreateElement("dhCont")
            dhelem.InnerText = Now.ToString("s")

            Dim justelem As XmlElement = nfeXML.CreateElement("xJust")
            justelem.InnerText = "Erro de conexão com o ambiente estadual"

            rootide.AppendChild(dhelem)
            rootide.AppendChild(justelem)

            'tirando o xmlns criado
            nfeXML.LoadXml(nfeXML.InnerXml.Replace(" xmlns=""""", ""))
            ...
            'salvando o transformado como transformado 2
            pathXmlTransformado = pathXmlTransformado.Replace("_transformado.xml", "_transformado_cont.xml")
            nfeXML.Save(pathXmlTransformado)

            Try
                nfeXML.PreserveWhitespace = True
                ...
                pathXmlTransformado = pathXmlTransformado.Replace("_transformado_cont.xml", "_assinado_cont.xml")
                nfeXML.Save(pathXmlTransformado)

                'inserirHistorico(6, "", nota, CNPJ)
            Catch ex As Exception
                Log.registrarErro("Erro inesperado " & vbCrLf & Geral.ObterExceptionMessagesEmCascata(ex) & vbCrLf & Geral.ObterStackTraceEmCascata(ex), "EnvioService")
                'inserirHistorico(7, "", nota, CNPJ)

                nota.retEnviNFe_xMotivo = "Erro no certificado digital (cont). Se já instalou o certificado e ainda apresenta este erro, acionar o helpdesk passando o CNPJ da sua empresa."
                nota.statusDaNota = 3
                notaDAO.alterarNota(nota)
                Continue For
            End Try
            ...
            'numerar o lote com o numero da nota
            ...
            'sincrono/assincrono quando subir a versão 3.0 somente
            ...
            Try
                ...
                ws = Nothing
            Catch ex As Exception
                'problema no envio normal, tentar novamente 
            ...
            If xmlRetorno.SelectSingleNode("/retEnviNFe/cStat").InnerText = 103 Then 'assincrono
                ...
                nota.statusDaNota = 52 'solicitar o retorno da nota

                'pega o recibo
                nota.retEnviNFe_infRec_nRec = xmlRetorno.SelectSingleNode("/retEnviNFe/infRec[1]/nRec[1]").InnerText
                notaDAO.alterarNota(nota)

                'TODO: Adicionar, se o serviço não estiver ativado, mudar nota para processar em ambiente normal
